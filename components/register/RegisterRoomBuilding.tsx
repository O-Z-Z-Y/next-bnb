...
const RegisterRoomBuilding: React.FC = () => {
  ...
  //* 빌딩 타입 리스트 리팩토링
  const setBuildingList = (buildingTypeList: any[]) => {
    dispatch(registerRoomActions.setBuildingType(buildingTypeList[0]));
  ...
  const detailBuildingOptions = useMemo(() => {
    switch (largeBuildingType) {
      case "아파트": {
        ...
        return setBuildingList(apartmentBuildingTypeList);
      }
      case "주택": {
        const { houseBuildingTypeList } = require("../../lib/staticData");
        return setBuildingList(houseBuildingTypeList)
      }
      case "별채": {
        const { secondaryUnitBuildingTypeList } = require("../../lib/staticData");
        return setBuildingList(secondaryUnitBuildingTypeList)
      }
      case "독특한 숙소": {
        const { uniqueSpaceBuildingTypeList } = require("../../lib/staticData");
        return setBuildingList(uniqueSpaceBuildingTypeList)
      }
      case "B&B": {
        const { bnbBuildingTypeList } = require("../../lib/staticData");
        return setBuildingList(bnbBuildingTypeList)
      }
      case "부티크호텔": {
        const { boutiqueHotelBuildingTypeList } = require("../../lib/staticData");
        return setBuildingList(boutiqueHotelBuildingTypeList)
      }
      default:
        return [];
    }
